...
subs_profile_source = PostgreSQLSource(
    ...
    query="""
        SELECT user_id, as_of, months_active, monthly_fee, paperless_billing, plan_stream_tv,  plan_stream_movies, net_service
        FROM subscriptions_profile_snapshots
    """,
    timestamp_field="as_of",  # TODO
)
...
usage_agg_30d_source = PostgreSQLSource(
    ...
    query="""
        SELECT user_id, as_of, watch_hours_30d, avg_session_mins_7d, unique_devices_30d, skips_7d, rebuffer_events_7d
...
payments_agg_90d_source = PostgreSQLSource(
    ...
    query="""
        SELECT user_id, as_of, failed_payments_90d
...
support_agg_90d_source = PostgreSQLSource(
    ...
    query="""
        SELECT user_id, as_of, support_tickets_90d, ticket_avg_resolution_hrs_90d

...
def main():
    ...
    # Note: Les préfixes (ex: 'stats:') dépendent de vos Feature Views
    features = [
    ...
    # TODO 3: Fusionner features + labels
    df = pd.merge(feat_df, labels, on=["user_id", "event_timestamp"])
    ...
    # TODO 4: Construire le préprocessing
    ...
    # TODO 5: Définir le modèle RandomForest
    clf = RandomForestClassifier(
        n_estimators=300,
        n_jobs=-1,
        random_state=42,  # Arbitraire fixé
        class_weight="balanced",
        max_features="sqrt",
    )

    # TODO 6: Définir la pipeline
    ...
    with mlflow.start_run(run_name=f"rf_baseline_{AS_OF}") as run:
        ...
        mlflow.log_metric("dataset length", len(df))

        # TODO 7: Logger un artefact JSON
        mlflow.log_dict(
            {"categorical_cols": cat_cols, "numeric_cols": num_cols},
            "feature_schema.json"
        )

        # TODO 8: Créer une signature MLflow et enregistrer le modèle
        input_schema = Schema([
        ...
        # TODO 9 : On log 'pipe' et non 'clf' pour inclure le preprocessing (OHE) dans l'artefact

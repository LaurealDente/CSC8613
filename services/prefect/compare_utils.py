import math

def should_promote(new_auc: float, prod_auc: float, delta: float = 0.01) -> bool:
    """
    Retourne True si le modèle candidat doit être promu.
    Règle : promotion si prod_auc est NaN (cas rare) OU si new_auc > prod_auc + delta.
    """

    if prod_auc is None or (isinstance(prod_auc, float) and math.isnan(prod_auc)) :
        return True
    
    if isinstance(prod_auc, float) and isinstance(new_auc, float) and isinstance(delta, float) and not math.isnan(prod_auc) and not math.isnan(new_auc):
        if new_auc > prod_auc + delta :
            return True
        
    return False
    
...
def upsert_csv(table: str, csv_path: str, pk_cols: list[str]):
    ...
    if not os.path.exists(csv_path):
        print(f"Warning: Fichier introuvable {csv_path}, on ignore.")
        return f"Skipped {table}"
    ...
    with eng.begin() as conn:
        ...
        # Construction de la clause SET col = EXCLUDED.col
        ...
        if non_pk:
            ...
            # Cas où il n'y a que la clé primaire (peu probable ici mais possible)
            sql = text(f"""
                INSERT INTO {table} ({collist})
                SELECT {collist} FROM {tmp}
...
def validate_with_ge(table: str):
    ...
    if table == "users":
        ...
        gdf.expect_table_columns_to_match_set([
            ...
            "net_service", "add_on_security", "add_on_backup", "add_on_device_protect", 
            "add_on_support"
        ])
        gdf.expect_column_values_to_not_be_null("user_id")
        ...
        # 1. Vérification que les colonnes correspondent exactement
        gdf.expect_table_columns_to_match_set([
            "user_id", 
            "watch_hours_30d", 
            "avg_session_mins_7d", 
        ...
        # 2. Vérification que l'ID n'est pas nul
        gdf.expect_column_values_to_not_be_null("user_id")
        
        # 3. Bornes raisonnables (valeurs >= 0 car ce sont des durées/compteurs)
...
def snapshot_month(as_of: str):
    """
    Crée (si besoin) les tables de snapshots et insère les données pour la date as_of donnée.
    Utilise une stratégie idempotente (ON CONFLICT DO NOTHING).
    """
    ddl = """
    ...
    sqls = [
        f"""
        INSERT INTO subscriptions_profile_snapshots (user_id, as_of, months_active, monthly_fee, paperless_billing, plan_stream_tv, plan_stream_movies, net_service)
        SELECT user_id, DATE '{as_of}', months_active, monthly_fee, paperless_billing, plan_stream_tv, plan_stream_movies, net_service
        FROM subscriptions
        ON CONFLICT (user_id, as_of) DO NOTHING;
        """,
        f"""
        INSERT INTO usage_agg_30d_snapshots (user_id, as_of, watch_hours_30d, avg_session_mins_7d, unique_devices_30d, skips_7d, rebuffer_events_7d)
        SELECT user_id, DATE '{as_of}', watch_hours_30d, avg_session_mins_7d, unique_devices_30d, skips_7d, rebuffer_events_7d
        FROM usage_agg_30d
        ON CONFLICT (user_id, as_of) DO NOTHING;
        """,
        # --- BLOC COMPLÉTÉ ICI ---
        f"""
        INSERT INTO payments_agg_90d_snapshots (user_id, as_of, failed_payments_90d)
        ...
        # -------------------------
        f"""
        INSERT INTO support_agg_90d_snapshots (user_id, as_of, support_tickets_90d, ticket_avg_resolution_hrs_90d)
